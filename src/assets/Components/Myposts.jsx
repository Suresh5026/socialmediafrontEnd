...
export default function Myposts() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        const response = await axios.get("https://socialmediaback-skiw.onrender.com/post/myPosts", {
          headers: {
  ...
  const handleDeletePost = async (postId) => {
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`https://socialmediaback-skiw.onrender.com/post/deletepost/${postId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
